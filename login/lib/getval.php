...
function caretbookopen($conn,$date) {
	$openbal = $this->getvalfield($conn,"caretcompanyopen","sum(openbalc)","1=1");
	$opendate = strtotime($this->getvalfield($conn,"company_setting","crtopendate","1=1"));
	$fromdatestr=strtotime($date);
	
	if($fromdatestr > $opendate) {
	//echo "hi"; die;	
$fromdate = $this->getvalfield($conn,"company_setting","crtopendate","1=1");
$date = date('Y-m-d', strtotime('-1 day', strtotime($date)));
//echo $date; die;

 $totalrec = $this->getvalfield($conn,"carretentry","sum(qty)","billdate between '$date' and '$date'  && is_sup=0"); 

 $othercaretin=  $this->getvalfield($conn,"othr_crt","sum(qty)","indate between '$date' and '$date'  && process='in'"); 

$purchaseinsup =  $this->getvalfield($conn,"purchaseentrydetail as A left join m_unit as B on A.unitid=B.unitid left join purchaseentry as C on A.purchaseid=C.purchaseid","sum(qty)","C.purchasedate between '$date' and '$date' && isstockable=1"); 

$totaldisc = $this->getvalfield($conn,"othr_crt","sum(qty)","indate between '$date' and '$date'  && process='out'");

$totalpayment = $this->getvalfield($conn,"saleenetry as A left join m_unit as B on A.unitid=B.unitid","sum(qty)","billdate between '$fromdate' and '$date' && isstockable=1") + $this->getvalfield($conn,"loaderentry as A left join m_unit as B on A.unitid=B.unitid left join loading as C on A.lodingid=C.lodingid","sum(qty)","C.loaddate between '$date' and '$date' && isstockable=1");

$suppret = $this->getvalfield($conn,"carretentry","sum(qty)","billdate between '$date' and '$date'  && is_sup=1");
...
function cashbookopen($conn,$date) {
$openbal = $this->getvalfield($conn,"company_setting","openbal","1=1");
$opendate = strtotime($this->getvalfield($conn,"company_setting","opendate","1=1"));
$fromdatestr=strtotime($date);

if($fromdatestr > $opendate) {
$fromdate = $this->getvalfield($conn,"company_setting","opendate","1=1");
$date = date('Y-m-d', strtotime('-1 day', strtotime($date)));
$totalrec = $this->getvalfield($conn,"payment","sum(paid_amt)","paymentdate between '$fromdate' and '$date'  && type='sale'");
$totincome = $this->getvalfield($conn,"other_income","sum(amount)","transdate between '$fromdate' and '$date'");
$totpayment = $this->getvalfield($conn,"payment","sum(paid_amt)","paymentdate between '$fromdate' and '$date' && type='purchase'");
$totexpense = $this->getvalfield($conn,"other_expense","sum(amount)","transdate between '$fromdate' and '$date'");
...
{
	$maxid = $this ->getvalfield($conn,$tblname,"max($tblpkey)","1=1");	
...
function InsertLogin($conn,$memberid, $emuid, $associationid, $asuid, $membertype, $username)
{
...
function InsertLog($conn,$pagename, $module, $submodule, $tablename, $tablekey, $keyvalue, $action)
{
	$sessionid = $this->getvalfield($conn,"m_session","session_name","status='1'");
...
function sendsms($smsuname,$msg_token,$smssender,$msg,$mobile)
{
	...
	//die;
	//First prepare the info that relates to the conn
...
{
	$getrow = mysqli_query($conn,$page_query);
...
{
	$sql = mysqli_query($conn,"set names utf-8 ");
...
function getcode($conn,$tablename,$tablepkey,$cond)
{
	$num =  $this->getvalfield($conn,$tablename,"max($tablepkey)","$cond");
...
function getrec($conn,$tablename,$tablepkey,$cond)
{
	$num =  $this->getvalfield($conn,$tablename,"max($tablepkey)","$cond");
	//if($num == NULL)
	//$num = 0;
...
function getdayname($name) {
	...
	function getopeningbalcust($conn,$suppartyid,$fromdate) {
	$openbal = $this->getvalfield($conn,"m_supplier_party","prevbalance","suppartyid='$suppartyid'");
	//$tilldate = date('Y-m-d', strtotime('-1 day', strtotime($fromdate)));
	$totbillamt = $this->getrounfsaleamt($conn,$fromdate,$suppartyid);		
	$totalpaid = $this->getvalfield($conn,"payment","sum(paid_amt + discamt)","suppartyid='$suppartyid' and paymentdate <='$fromdate' and type='sale'");
	
	$totpur = $this->getpurchasetilldate($conn,$fromdate,$suppartyid);
	$purpay = $this->getvalfield($conn,"payment","sum(paid_amt + discamt)","suppartyid='$suppartyid' and paymentdate <='$fromdate'  and type='purchase'");
	
	$loadamt = $this->getvalfield($conn,"loading","sum(netsale)","suppartyid='$suppartyid' and loaddate <='$fromdate'");
	
	return $openbal + $totbillamt - $totalpaid - $totpur + $purpay + $loadamt;
	}
	
	function getpurchasetilldate($conn,$fromdate,$suppartyid) {
	$pur=0;
	$sql = mysqli_query($conn,"select purchaseid from purchaseentry where purchasedate <= '$fromdate' && suppartyid='$suppartyid'");
	while($row=mysqli_fetch_assoc($sql)) {
	$pur +=$this->gettotalpurchase($conn,$row['purchaseid']);
	}
	...
	function gettotalpurchase($conn,$purchaseid) {
	
	$netamount = 0;
	$total_unit_rate=0;
	$sql = mysqli_query($conn,"select * from purchaseentrydetail where purchaseid='$purchaseid'");
	while($row=mysqli_fetch_assoc($sql)) {
	...
	$tot_exp = 0;
	$sql = mysqli_query($conn,"select * from purchaseexp where purchaseid='$purchaseid'");
	while($row=mysqli_fetch_assoc($sql))
	{
	...
	function getrounfsaleamt($conn,$date,$suppartyid) {
	$saleamount=0;
	
	$sql = mysqli_query($conn,"select billdate from saleenetry where suppartyid='$suppartyid' and billdate <='$date' group by billdate");
	while($row=mysqli_fetch_assoc($sql))	
	{
	$billdate = $row['billdate'];		
	$saleamount += round($this->getvalfield($conn,"saleenetry","sum((qty * rate * weight)+(qty * unitrate)+other_charges)","suppartyid='$suppartyid' and billdate ='$billdate'"));
	}
	return $saleamount;
...
function getcarretopenbydate($conn,$suppartyid,$unitid,$date) {
		
		$openbalcar = $this->getvalfield($conn,"openingunitentry","openunit","suppartyid='$suppartyid' and unitid='$unitid' && type=0");			
		$qtycartoon = $this->getvalfield($conn,"saleenetry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate <='$date'"); 
	    $retaty = $this->getvalfield($conn,"carretentry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate <='$date' && is_sup=0");
		
		 $purqtycartoon = $this->getvalfield($conn,"purchaseentrydetail as A left join purchaseentry as B on A.purchaseid=B.purchaseid","sum(A.qty)","B.suppartyid='$suppartyid' and A.unitid='$unitid' && purchasedate <='$date'");  
		$purretaty = $this->getvalfield($conn,"carretentry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate <='$date' && is_sup=1");
		
		$loadingcaret = $this->getvalfield($conn,"loaderentry as A left join loading as B on A.lodingid=B.lodingid","sum(A.qty)","B.suppartyid='$suppartyid' and A.unitid='$unitid' && loaddate <='$date'");  
...
function getcarretopenbydate2($conn,$suppartyid,$unitid,$date) {
...
function getcarretopenbydatebetween($conn,$suppartyid,$unitid,$date,$todate) {
		
	$openbalcar = $this->getvalfield($conn,"openingunitentry","openunit","suppartyid='$suppartyid' and unitid='$unitid' && type=0");			
	$qtycartoon = $this->getvalfield($conn,"saleenetry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate between '$date' and '$todate'"); 
	$retaty = $this->getvalfield($conn,"carretentry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate between '$date' and '$todate' && is_sup=0");
	
	$purqtycartoon = $this->getvalfield($conn,"purchaseentrydetail as A left join purchaseentry as B on A.purchaseid=B.purchaseid","sum(A.qty)","B.suppartyid='$suppartyid' and A.unitid='$unitid' && purchasedate between '$date' and '$todate'");

	$purretaty = $this->getvalfield($conn,"carretentry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate between '$date' and '$todate' && is_sup=1");
	
	$loadingcaret = $this->getvalfield($conn,"loaderentry as A left join loading as B on A.lodingid=B.lodingid","sum(A.qty)","B.suppartyid='$suppartyid' and A.unitid='$unitid' && loaddate between '$date' and '$todate'");  
...
function gettotalloadamt($conn,$lodingid) {
	
	$total = $this->getvalfield($conn,"loading","netsale","lodingid='$lodingid'");
	return $total;
...
while($row_get = mysqli_fetch_assoc($sql_get))
	{
		...
		$msg='';		
		$sql = mysqli_query($conn,"select paymentid from payment where suppartyid='$suppartyid' and paymentdate='$row_get[date]' && type='sale'");
			while($row=mysqli_fetch_assoc($sql)) {	
				$paymentid = $row['paymentid'];			
			$payamount = $this->getvalfield($conn,"payment","sum(paid_amt + discamt)",
			"paymentid='$paymentid'");				
					if($payamount !=0) {
					$msg ="Receipt" ;
					}				
			...
			$msg='';
			$puramt = 0;			
			$sql = mysqli_query($conn,"select * from purchaseentry where suppartyid='$suppartyid' and purchasedate = '$row_get[date]'");
			while($row=mysqli_fetch_assoc($sql)) {
			$puramt = $this->gettotalpurchase($conn,$row['purchaseid']);
			if($puramt !=0) {
			 $msg ="Bijak";
			...
			$msg='';
			$sql = mysqli_query($conn,"select paymentid,paid_amt,discamt from payment where suppartyid='$suppartyid' and paymentdate='$row_get[date]' && type='purchase'");
			while($row=mysqli_fetch_assoc($sql)) {	
				$paymentid = $row['paymentid'];			
				...
				$sql = mysqli_query($conn,"select lodingid from loading where suppartyid='$suppartyid' and loaddate = '$row_get[date]'");
				$numrow = mysqli_num_rows($sql);
				while($row=mysqli_fetch_assoc($sql)) {
						//$lodingid = $row['lodingid'];
						$loadamt = $this->gettotalloadamt($conn,$row['lodingid']);
...
function getcustomerroaker($conn,$suppartyid,$fromdate,$todate) {
...
$roaker= array();	
	...
	while($row_get = mysqli_fetch_assoc($sql_get))
				...
				$payamount = $this->getvalfield($conn,"payment","sum(paid_amt + discamt)","suppartyid='$suppartyid' and paymentdate='$row_get[date]' && type='sale'");
				...
				if($msg !='') {
				//$nettoral = $this->getopeningcustomer($conn,$row_get['date'],$suppartyid);
				$nettoral=0;
			...
			$sql = mysqli_query($conn,"select * from purchaseentry where suppartyid='$suppartyid' and purchasedate = '$row_get[date]'");
			while($row=mysqli_fetch_assoc($sql)) {
			$total = $this->gettotalpurchase($conn,$row['purchaseid']);
			$puramt += $total;
			}
				
				$paidamt= $this->getvalfield($conn,"payment","sum(paid_amt + discamt)","suppartyid='$suppartyid' && type='purchase' and paymentdate='$row_get[date]'");	
				...
				while($row=mysqli_fetch_assoc($sql)) {
						//$lodingid = $row['lodingid'];
						$total = $this->gettotalloadamt($conn,$row['lodingid']);
						$loadamt += $total;
				}
...
function getpurexpense($conn,$purchaseid,$addexp_id) {
	$netamount = 0;
	$total_unit_rate=0;
	$sql = mysqli_query($conn,"select * from purchaseentrydetail where purchaseid='$purchaseid'");
	while($row=mysqli_fetch_assoc($sql)) {
	...
	$sql = mysqli_query($conn,"select * from purchaseexp where purchaseid='$purchaseid' && addexp_id='$addexp_id'");
	while($row=mysqli_fetch_assoc($sql))
	{
...
function getcarretroaker($conn,$suppartyid,$fromdate,$todate) {

$nettoral= 0;
$roaker= array();	

$sql_get = mysqli_query($conn,"Select Distinct(date) From (
...
while($row_get = mysqli_fetch_assoc($sql_get))
{
	...
	$supparray=array();	
	$loadarray =array();	
	$sql = mysqli_query($conn,"select unitid,unit_name,unit_name_hindi from m_unit where isstockable=1");
				while($row=mysqli_fetch_assoc($sql)) {
			...
			$cartbalopen = $this->getcarretopenbydate($conn,$suppartyid,$unitid,$dateopen);
			
					 $salecaret = $this->getvalfield($conn,"saleenetry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate='$row_get[date]'");
					 $recaret = $this->getvalfield($conn,"carretentry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate='$row_get[date]' && is_sup=0");
					 
					 $returncaret = $this->getvalfield($conn,"carretentry","sum(qty)","suppartyid='$suppartyid' and unitid='$unitid' && billdate='$row_get[date]' && is_sup=1");
					 
					 $purcaret  = $this->getvalfield($conn,"purchaseentrydetail as A left join purchaseentry as B on A.purchaseid=B.purchaseid","sum(qty)","suppartyid='$suppartyid' and A.unitid='$unitid' && purchasedate='$row_get[date]'");
					 
					  $loadcaret  = $this->getvalfield($conn,"loaderentry as A left join loading as B on A.lodingid=B.lodingid","sum(qty)","suppartyid='$suppartyid' and A.unitid='$unitid' && loaddate='$row_get[date]'");
...
function getcarretroakerall($conn,$suppartyid,$fromdate,$todate,$unitid) {
...
while($row_get = mysqli_fetch_assoc($sql_get))
{
	...
	$supparray=array();	
	$loadarray =array();	
	$sql = mysqli_query($conn,"select unitid,unit_name,unit_name_hindi from m_unit where isstockable=1 && unitid='$unitid' limit 1");
		while($row=mysqli_fetch_assoc($sql)) {
		...
		$sc= mysqli_query($conn,"select * from carretentry where suppartyid='$suppartyid' and unitid='$unitid' && billdate='$row_get[date]' && is_sup=1");
	...
	$custarray[]=array($unit_name,$process,$msg,$returncaret,'',$returncaret,$net,$row_get['date']);					
		...
		$sc= mysqli_query($conn,"select A.* from loaderentry as A left join loading as B on A.lodingid=B.lodingid where suppartyid='$suppartyid' and A.unitid='$unitid' && loaddate='$row_get[date]'");
					while($rc=mysqli_fetch_assoc($sc)) {
					$process="Loader";
	...
	$sc= mysqli_query($conn,"select * from carretentry where suppartyid='$suppartyid' and unitid='$unitid' && billdate='$row_get[date]' && is_sup=0");
		while($rc=mysqli_fetch_assoc($sc)) {
		$process="Customer";
		$recno = $rc['recno'];
		if($recno!='')
		...
		$sc= mysqli_query($conn,"select A.* from purchaseentrydetail as A left join purchaseentry as B on A.purchaseid=B.purchaseid where suppartyid='$suppartyid' and A.unitid='$unitid' && purchasedate='$row_get[date]'");
			while($rc=mysqli_fetch_assoc($sc)) {
			$process="Supplier";
...
function getcarretroakerall2($conn,$suppartyid,$fromdate,$todate,$unitid) {
